...
1. Checkout the issue before starting any work:
   \`curl -s -X POST "{{paperclipApiUrl}}/issues/{{taskId}}/checkout" -H "Authorization: Bearer $PAPERCLIP_API_KEY" -H "X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID" -H "Content-Type: application/json"\`
   If you get a 409, another agent owns this task — stop and exit.

2. Load full task context:
   \`curl -s "{{paperclipApiUrl}}/issues/{{taskId}}/heartbeat-context" -H "Authorization: Bearer $PAPERCLIP_API_KEY" | jq .\`
...
6. If this issue has a parent, notify the parent:
   \`curl -s -X POST "{{paperclipApiUrl}}/issues/PARENT_ISSUE_ID/comments" -H "Authorization: Bearer $PAPERCLIP_API_KEY" -H "X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID" -H "Content-Type: application/json" -d '{"body":"{{agentName}} completed subtask {{taskId}}. Summary: <brief>"}'\`
{{/taskId}}
...
Someone commented. Read it:
   \`curl -s "{{paperclipApiUrl}}/issues/{{taskId}}/comments/{{commentId}}" -H "Authorization: Bearer $PAPERCLIP_API_KEY" | jq .\`
...
1. Check your inbox for assigned tasks:
   \`curl -s "{{paperclipApiUrl}}/agents/me/inbox-lite" -H "Authorization: Bearer $PAPERCLIP_API_KEY" | jq .\`

2. If issues found, pick the highest priority in_progress task first, then todo. For each:
   a. Checkout: \`curl -s -X POST "{{paperclipApiUrl}}/issues/ISSUE_ID/checkout" -H "Authorization: Bearer $PAPERCLIP_API_KEY" -H "X-Paperclip-Run-Id: $PAPERCLIP_RUN_ID" -H "Content-Type: application/json"\`
      If you get a 409, skip this task — it belongs to another run.
   b. Load context: \`curl -s "{{paperclipApiUrl}}/issues/ISSUE_ID/heartbeat-context" -H "Authorization: Bearer $PAPERCLIP_API_KEY" | jq .\`
   c. Do the work, then update status and post a comment when done (see steps 4-5 above).

3. If inbox is empty, exit cleanly. Do NOT scan for unassigned issues or self-assign work.
{{/noTask}}`;
